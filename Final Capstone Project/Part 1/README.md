...
- 32 records shared duplicate `order_id` values with conflicting data — these were flagged in the `duplicate_flag` column and retained for review (first occurrence treated as canonical)

### Step 4 – Missing Value Handling
- `region` (26 nulls) → filled as `Unknown`
- `ship_mode` (22 nulls) → filled as `Unknown`
- `discount` (18 nulls) → filled as `0` where other financial fields were valid

### Step 5 – Discount Validation
- 16 records had negative discounts → flagged as Invalid
...
| Conflicting duplicate order_ids flagged | 32 |
